...
class SK_Alvis_XML_Import {
	...
	public function get_xml() {
			...
			return array(
				'result'  => false,
				'message' => 'Kunde inte komma åt xmlfilen. Anslutningsfel eller filen finns inte.'
			);
	...
	private function update_course_terms( $post_id, $course ) {
		...
		if ( ! in_array( $course_type, $collected_terms['studieform'] ) ) {
			array_push( $collected_terms['studieform'], $course_type );
		}
		...
		if ( ! in_array( $level, $collected_terms['niva'] ) ) {
			array_push( $collected_terms['niva'], $level );
		}
		...
		if ( count( $terms ) > 0 ) {
			foreach ( $terms as $tmp_term ) {
				if ( ! in_array( $tmp_term, $collected_terms['amnesomrade'] ) ) {
					array_push( $collected_terms['amnesomrade'], $tmp_term );
				}
			}
		}
		...
		if ( ! in_array( (string) $course->poäng, $collected_terms['poang'] ) ) {
			array_push( $collected_terms['poang'], (string) $course->poäng );
		}
	...
	public function add_course_import_options() {
		...
		add_options_page( __( 'Inställningar för Alvis kursimport', 'sk' ), __( 'Kursimport', 'sk' ), 'edit_pages', 'sk_course_import_options', array(
			$this,
			'course_import_options'
		) );
		...
		<div class="wrap">
			...
			<?php if ( current_user_can( 'activate_plugins' ) ) : ?>
				<form method="post" action="options.php">
					...
					<p><input type="submit" class="button button-primary" name="Submit"
					          value="<?php _e( 'Spara' ); ?>"/></p>
